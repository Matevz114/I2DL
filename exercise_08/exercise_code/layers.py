...
def batchnorm_backward(dout, cache):
    ...
    #????dx = gamma * x_minus_mean * (ivar ** 3) * (-0.5) * (2/(N**2)) * x_minus_mean
    #????dx = gamma * (ivar + ivar / N + x_minus_mean * (-0.5) * ivar**3 * 2/N * x_minus_mean)

    dx = (1. / N) * gamma * ivar * (
    N * dout
    - np.sum(dout, axis=0)
    - x_minus_mean * ivar**2 * np.sum(dout * x_minus_mean, axis=0)
)
