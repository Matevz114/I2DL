...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self,
        ...
        ########################################################################
        q_linear = self.weights_q(q)
        k_linear = self.weights_k(k)
        v_linear = self.weights_v(v)

        q_reshaped = q_linear.reshape(batch_size, sequence_length_queries, self.n_heads, self.d_k).transpose(1, 2)
        k_reshaped = k_linear.reshape(batch_size, sequence_length_keys, self.n_heads, self.d_k).transpose(1, 2)
        v_reshaped = v_linear.reshape(batch_size, sequence_length_keys, self.n_heads, self.d_v).transpose(1, 2)

        outputs = self.attention(q_reshaped, k_reshaped, v_reshaped)
