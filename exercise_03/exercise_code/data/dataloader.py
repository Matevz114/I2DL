...
class DataLoader:
    ...
    def __iter__(self):
        ...
        if self.shuffle:
            shuffled_indices = list(np.random.permutation(len(self.dataset)))
            index_iterator = iter(shuffled_indices)
            #iter(np.random.permutation(len(self.dataset)))
        else:
        ...
        for index in index_iterator:
            ...
            if len(batch) == self.batch_size:
                ...
                batch = []
#
        ##additional logic for droplast = false
        ##need remainder: r = len(dataset) mod batchsize
        ##take last r indexes in index_iterator: starting at index_iterator[-r] and append dataset[-r], dataset[-r + 1].... dataset[-1]
        if self.drop_last == False:
            batch = []
            r = len(self.dataset) % self.batch_size
            last_indices = iter(shuffled_indices[-r:])
            if r > 0:
